...
def generate_road_network_adjacency_matrix(nbr_cities: int, max_distance: int) -> [[int]]:
    ...
    # Make sure to copy the array each time to not get the same reference for each row of the 2d list
...
def generate_random_solution(nbr_cities: int) -> [int]:
    ...
    :return: List of the cities reached by order for  the generated path
    """
